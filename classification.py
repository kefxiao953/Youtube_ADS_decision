#======================Classification==============================
#Classification是指分类一般是依据多个feature来对label进行标注，常见的应用有：CV计算机视觉和NLP自然语言处理
#Classification也有多种子任务，比如二分类，多分类，多标签分类等等。
#本项目提供的数据集，很适合进行catefory的多分类任务，因此我们用一章内容来实现一个分类
#请注意，我们此处进行的分类不是为了解决投流对象问题，而是一个单独的任务，
#比如：训练一个分类器，能够根据文字描述，自动识别是哪一个category。
#这一类任务经常出现在金融防欺诈检测中，在本项目中出现，更多的是为了后续可能存在人工智能应用打下基础。
#====================================================
# 引入模块
import pandas as pd
import pandas as pd
...
#================================================================
#观察结果，你是否觉得accuracy_score比使用Logistic regression 高了很多？
#这就是面对不同的任务，选用合适的模型，带来的结果提升

#================================================================
#练习：不抄答案，从train/test spliting开始， 使用xgboost实现这个任务，并给出accuracy score
